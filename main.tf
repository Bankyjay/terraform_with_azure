...
resource "azurerm_virtual_network" "main" {
  ...
  address_space       = [element(var.network_config, 0)] # or var.network_config[0]
...
resource "azurerm_subnet" "internal" {
  ...
  address_prefixes     = ["${element(var.network_config, 1)}/${element(var.network_config, 1)}"]
}
...
resource "azurerm_virtual_machine" "main" {
  ...
  #impelementing maps data type for tags 
  tags = {
    environment = var.resource_tags["environment"]
    managed_by  = var.resource_tags["managed_by"]
    team        = var.resource_tags["team"]
  }
}
